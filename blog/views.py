...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = f"{cd['name']} recomends you " + f"{post.title}"
            message = f"Read {post.title} at {post_url}" + f"{cd['name']}\'s comments: {cd['comments']}"
            send_mail(subject, message, 'mysitedemo2022@gmail.com', [cd['to']])
            sent = True
    else:
        form = EmailPostForm()

    context = {'post':post ,'form':form, 'sent':sent}
    return render(request, 'blog/post/share.xhtml', context)
...
def post_comment(request, post_id):
    ...
    return render(request, 'blog/post/comment.xhtml', context)
...
def post_search(request):
    ...
    return render(request, 'blog/post/search.xhtml', context)
